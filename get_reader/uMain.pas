...
ErrCode:= 0;
  If ErrCode = 0 Then Begin
	 If InBuf[0] = $00 Then Begin  // Версия ПО
		MessLine := 'Версия ПО: ' + IntToStr(InBuf[2]) + '.' + FormatFloat('00', InBuf[3]);
		RzMTest.Lines.Add(MessLine);
		if bAuto then 
			FMain.AGetDate.Execute; // если авто - далее
	 End Else If InBuf[0] = $01 Then Begin
    ...
    End Else If InBuf[0] = $02 Then Begin
		MessLine := 'Дата/время установлены.';
      RzMTest.Lines.Add(MessLine);
		if bAuto then FMain.AOraConnect.Execute; // если авто - далее
	 End Else If InBuf[0] = $03 Then Begin  // Прочитано количество в текущей базе
