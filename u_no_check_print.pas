...
begin
  ...
  ShowMessage('Не выбраны обязательные параметры!');
	 ...
	 DM.OQNoCheckPrint.SQL.Text := DM.OQNoCheckPrintDefault.SQL.Text;  //запрос по-умолчанию
    ...
    DM.OQNoCheckPrint.ParamByName('otd').AsInteger := FMain.RzRegIniFile1.ReadInteger('Sets', 'Otd', 1);;
	 ...
	 If FMain.RzCheckBox10.Checked then //фильтруем по типу счётчиков если стоит галочка на главной форме
		DM.OQNoCheckPrint.SQL.Add(' AND ste.INTER_TYPE= ' + IntToStr(FMain.RzDBLCBTypeSch.KeyValue)); 
	 
	 //DM.OQNoCheckPrint.SQL.Add(' ORDER BY u.NAME, st.NAME_STREET, l.DOM, l.KORP, l.FLAT');
      DM.OQNoCheckPrint.SQL.Add(' ORDER BY 1, 2, 3, 4, 5');
